...
export const treasurehunt = async (req, res) => {
    try {
        ...
        const existingRegistration = await TreasureHunt.findOne({ email: req.body.email });

        if (existingRegistration) return res.status(400).json({ message: "Email is already registred." });

        const savedData = await TreasureHunt.create(req.body);
