...
export const cultural = async (req, res) => {
    try {
        ...
        const existingRegistration = await Cultural.findOne({ email: req.body.email });

        if (existingRegistration) return res.status(400).json({ message: "Email is already registred." });

        const savedData = await Cultural.create(req.body);
