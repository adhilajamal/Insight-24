...
export const webdesigning = async (req, res) => {
    try {
        ...
        const existingRegistration = await WebDesigning.findOne({ email: req.body.email });

        if (existingRegistration) return res.status(400).json({ message: "Email is already registred." });

        const savedData = await WebDesigning.create(req.body);
